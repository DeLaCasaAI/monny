...
export const BudgetHeader: React.FC<BudgetHeaderProps> = ({ budget, onBack, onEdit }) => {
  ...
  const getBudgetIcon = (type: Budget['type']) => {
    switch (type) {
      case 'business':
        return <Building2 className="h-6 w-6 text-blue-600 dark:text-blue-400" />;
      case 'trip':
        return <Plane className="h-6 w-6 text-blue-600 dark:text-blue-400" />;
      default:
  ...
  return (
    <div className="mb-6">
      <Button 
        onClick={onBack}
        variant="ghost"
        className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mb-4 p-0"
      >
